...
class MapIntersections:
    ...
    def output(xCoord, yCoord, nPoints, model):
        ...
        def computeInterface(index):
            ...
            yBoundaryList[startIndex:startIndex + nPoints - 1] = list(
                map(findIntersectionY, rankMatrix[index][1:nPoints], rankMatrix[index][0:nPoints - 1], yMapRange[0:nPoints - 1], yMapRange[1:nPoints], np.ones(nPoints) * xMapRange[index]))
            ...
            xBoundaryList[startIndex + nPoints:startIndex + nPoints * 2] = list(
                map(findIntersectionX, rankMatrix[:][index], rankMatrix[:][index + 1], np.ones(nPoints) * xMapRange[index], np.ones(nPoints) * xMapRange[index + 1], yMapRange))
...
class CrossSectionIntersections:
    ...
    def output(xCoord, yCoord, zCoord, nPoints, model, imgSize, offSet):
        ...
        def computeInterface(index):
            ...
            startIndex = index * nPoints + (nPoints - 1) * index
            yBoundaryList[startIndex:startIndex + nPoints - 1] = list(
                map(findIntersectionY, rankMatrix[index][1:nPoints], rankMatrix[index][0:nPoints - 1], zCrossSectionRange[0:nPoints - 1], zCrossSectionRange[1:nPoints],
                    np.ones(nPoints) * xCrossSectionRange[index]))
            xBoundaryList[startIndex:startIndex + nPoints - 1] = np.ones(nPoints - 1) * xCrossSectionRange[index]
            minimalRanksList[startIndex:startIndex + nPoints - 1] = list(map(np.minimum, rankMatrix[index][0:nPoints - 1], rankMatrix[index][1:nPoints]))

            xBoundaryList[startIndex + nPoints:startIndex + nPoints * 2] = list(
                map(findIntersectionX, rankMatrix[:][index], rankMatrix[:][index + 1], np.ones(nPoints) * xCrossSectionRange[index], np.ones(nPoints) * xCrossSectionRange[index + 1],
                    zCrossSectionRange))
...
class MapSlice:
    @staticmethod
    def output(xCoord: List[int], yCoord: List[int], nPoints: int, ranks: Callable[[Any], Any], evaluate_z: Callable[[Any], Union[float, np.ndarray]]):
        def computeRank(a, b):
            return ranks([a, b, evaluate_z([a, b])])
